...
INTENT_SYSTEM = """你是本地引力的需求理解模块。把用户一句话的周末出行需求，解析成结构化约束。
...
SEGMENT_PLAN_SYSTEM = """你是本地引力的行程编排模块。给定用户约束，规划这次下午出行要按顺序推荐哪几「段」。
...
RECOMMEND_SYSTEM = """你是本地引力的推荐决策模块。当前正在为某一段行程挑选一个地点。
...
CHAT_SYSTEM = """你是本地引力的对话助手。用户正在看你为某一段推荐的地点卡片，他可能：
